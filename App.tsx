...
// ÖNEMLİ: KENDİ GEMINI API ANAHTARINI AŞAĞIDAKİ TIRNAKLARIN İÇİNE YAPIŞTIR!
// Eğer anahtarın yoksa https://aistudio.google.com/app/apikey adresinden alabilirsin.
...
const generateStoryText = async (input: UserInput): Promise<StoryData> => {
  try {
    ...
    if (response.status === 403) {
        throw new Error("API Anahtarı geçersiz veya eksik. Lütfen App.tsx dosyasındaki API_KEY alanını kontrol edin.");
    }
...
const generateIllustration = async (prompt: string): Promise<string> => {
  try {
    ...
    if (response.status === 403) {
        console.warn("Resim servisi 403 hatası verdi. Placeholder kullanılıyor.");
        return "https://via.placeholder.com/512?text=Resim+Servisi+Hatasi";
    }
...
function App() {
  ...
  const handleFormSubmit = async (input: UserInput) => {
    ...
    try {
      ...
      // GÜNCELLEME: Hata detayını ekranda görmek için bu kısmı değiştirdik.
      // Sorunu çözdükten sonra eski "Sihirli bir hata..." mesajına dönebilirsin.
      const debugMessage = err instanceof Error ? err.message : JSON.stringify(err);
      setErrorMsg(`Teknik Hata: ${debugMessage}`);
